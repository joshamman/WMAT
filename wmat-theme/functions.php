...
if ( ! function_exists( 'wmat_enqueue_assets' ) ) {
	...
	function wmat_enqueue_assets() {
		...
		wp_enqueue_script(
			'wmat-script',
			get_template_directory_uri() . '/assets/js/wmat.js',
			array(),
			$version,
			true
		);
...
if ( ! function_exists( 'wmat_register_pattern_category' ) ) {
	...
	function wmat_register_pattern_category() {
		...
		register_block_pattern_category(
			'page',
			array(
				'label'       => __( 'Full pages', 'wmat' ),
				'description' => __( 'Complete page layouts for the WMAT site.', 'wmat' ),
			)
		);
	}
}
...
if ( ! function_exists( 'wmat_icon' ) ) {
	/**
	 * Return an inline Lucide stroke icon, sized by the parent's font-size
	 * (CSS makes the <svg> 1em). Used inside block patterns.
	 *
	 * @param string $name   Icon name.
	 * @param float  $stroke Stroke width.
	 * @return string Safe inline SVG markup.
	 */
	function wmat_icon( $name, $stroke = 1.9 ) {
		$paths = array(
		...
		return sprintf(
			'<span class="wmat-ico" aria-hidden="true"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="%s" stroke-linecap="round" stroke-linejoin="round">%s</svg></span>',
