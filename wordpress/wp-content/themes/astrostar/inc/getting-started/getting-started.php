        ...
        if ( is_admin() && !get_user_meta( get_current_user_id(), 'gs_notice_dismissed' ) ){
            echo '<div class="updated notice notice-success is-dismissible getting-started getting-started-wrap">';
                    echo '<div class="getting-content">';
                        echo ( '<h2>' . sprintf( esc_html__( 'Welcome to %1$s Theme', 'astrostar' ), esc_attr ( $theme->get( 'Name' ) ) ) . '</h2>' );
                        echo '<h3>To fully take advantage of the best our theme can offer, get started.</h3>';
                        echo ( '<p><a href="'.esc_url( admin_url( 'themes.php?page=wpdevart-theme-about' ) ).'" class="button button-secondary getting-started-main-button">' . sprintf( esc_html__( 'Get started with %s','astrostar' ), esc_attr ( $theme->get( 'Name' ) ) ) . '</a>' );
                        echo ( '<a href="'.apply_filters( 'parent_wpdevart_astrostar_premium_features_url', esc_url('https://wpdevart.com/astrostar-wordpress-astrology-theme')).'" class="button button-secondary getting-started-main-button getting-started-focus-button" target="_blank">' . sprintf( esc_html__( '%s Premium','astrostar' ), esc_attr ( $theme->get( 'Name' ) ) ) . '</a></p>' );
                        echo ( '<p class="plugin-notice">'.esc_html__( 'To get started, take action and learn more about our theme. If you want to hide this notice, click the Dismiss button in the top right corner.', 'astrostar' ).'</p>' );
                    echo '</div>';
